...
// TestNoBrowserSurface proves the profile's point: the API-only service
// answers the JSON API and auth, and nothing else. There is no landing page,
// no dashboard, no fragment and no static asset.
func TestNoBrowserSurface(t *testing.T) {
	ts := newTestServer(t)

	for _, path := range []string{"/", "/app", "/notes", "/login", "/signup", "/ui/notes", "/static/css/tokens.css"} {
		resp, raw := get(t, ts, path, "")
		assert.Equal(t, http.StatusNotFound, resp.StatusCode, "GET %s: %s", path, raw)
	}
}
