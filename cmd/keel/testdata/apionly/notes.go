...
// Create inserts a note owned by userID and returns it as stored, with the id
// and timestamp the database assigned rather than ones guessed here.
func (n *Notes) Create(ctx context.Context, userID, title, body string) (Note, error) {
	var note Note
...
// Delete removes one of userID's notes, reporting ErrNoteNotFound when there
// was nothing to remove. A delete that silently succeeds against a missing
// row cannot be told apart from one that worked.
func (n *Notes) Delete(ctx context.Context, userID, id string) error {
	tag, err := n.pool.Exec(ctx, `DELETE FROM notes WHERE id = $1 AND user_id = $2`, id, userID)
	if err != nil {
...
// userIDField is both the notes column and the indexed document's field. The
// search handler filters on it so a caller only ever sees their own rows.
...
// List returns a page of userID's notes newest first, plus the cursor for the
// next page. An empty cursor means there is no next page.
func (n *Notes) List(ctx context.Context, userID string, limit int, cursor string) ([]Note, string, error) {
	...
	// $1 is the owner; the seek predicate numbers its placeholders from $2.
	where, args, err := keyset.Where(2)
	if err != nil {
		return nil, "", err
	}

	query := `SELECT id, user_id, title, body, created_at FROM notes WHERE user_id = $1`
...
// All reads every note, with its owner. Used by the reconcile job, which needs
// the whole table to decide what the search index should contain; the index
// carries the owner so a search can filter to the caller's own rows.
func (n *Notes) All(ctx context.Context) ([]Note, error) {
	rows, err := n.pool.Query(ctx, `SELECT id, user_id, title, body, created_at FROM notes`)
	if err != nil {
